...
**Difficulty:** Hard  
**LeetCode:** [Design In-Memory File System](https://leetcode.com/problems/design-in-memory-file-system/) (Premium)
...
- `ls("/")` → `[]` — root is empty
- `mkdir("/a/b/c")` — create directory `/a/b/c` (and `/a`, `/a/b` as needed)
- `addContentToFile("/a/b/c/d", "hello")` — create file `d` with content `"hello"`
- `ls("/")` → `["a"]` — root now contains directory `a`
...
3. You can assume all file or directory paths are **absolute paths** which begin with `/` and do not end with `/` except that the path is just `"/"`.

---

## Project structure

- **`file_system.py`** — `FileSystem` class (implement here)
- **`test_cases.py`** — test case definitions
- **`driver.py`** — runs tests: `python driver.py`
